    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          # Exact variable names used in the exercise boilerplate
        ...
        comparator = function(generated, expected, ...) {
          ...
          wrong_msg <- function(key) {
            r <- results[[key]]
            if (!r$exists) {
              return("Je hebt deze variabele niet ingevuld.")
            }
            if (is.na(r$value_num)) {
              return("Je antwoord kon niet als getal geïnterpreteerd worden.")
            }

            # Dispatch to specific diagnostic functions
            if (key == "perc_male") {
              return(wrong_msg_perc_male(r$value))
            }
            if (key == "perc_female") {
              return(wrong_msg_perc_female(r$value))
            }
            if (key == "perc_yes") {
              return(wrong_msg_perc_yes(r$value))
            }
            if (key == "perc_no") {
              return(wrong_msg_perc_no(r$value))
            }
            if (key == "perc_yes_male") {
              return(wrong_msg_perc_yes_male(r$value))
            }
            if (key == "perc_yes_female") {
              return(wrong_msg_perc_yes_female(r$value))
            }
            if (key == "perc_diff_yes") {
              return(wrong_msg_perc_diff(r$value))
            }
            if (key == "odds_male") {
              return(wrong_msg_odds_male(r$value))
            }
            if (key == "odds_female") {
              return(wrong_msg_odds_female(r$value))
            }
            if (key == "odds_ratio") {
              return(wrong_msg_odds_ratio(r$value))
            }
            if (key == "chi2") {
              return(wrong_msg_chi2(r$value))
            }
            if (key == "p_no_female") {
              return(wrong_msg_p_no_female(r$value))
            }
            if (key == "antwoord_mc") {
              return(wrong_msg_mc(r$value))
            }
