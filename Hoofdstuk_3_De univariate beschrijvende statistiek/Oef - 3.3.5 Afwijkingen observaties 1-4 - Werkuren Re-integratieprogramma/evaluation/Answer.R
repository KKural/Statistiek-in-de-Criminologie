    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          assign("results_3_3d", results, envir = globalenv())
          all(vapply(results, function(result) isTRUE(result$ok), logical(1)))
        },
        TRUE,
        comparator = function(generated, expected, ...) {
          results <- get("results_3_3d", envir = globalenv())
          deviations <- results$deviations
          squared <- results$squared
          if (isTRUE(generated == expected)) {
            message <- paste(
              "**Bevestiging:** de vier afwijkingen en vier gekwadrateerde afwijkingen voor observaties 1 tot en met 4 zijn correct.",
              "**Denkregel:** afwijking = x − gemiddelde; kwadrateer de ongeronde getekende afstand en rond pas het eindresultaat af.",
              "**Transferstap:** gebruik dezelfde twee bewerkingen voor het volgende blok observaties en behoud de datavolgorde.",
              sep = "\n\n"
            )
          } else {
