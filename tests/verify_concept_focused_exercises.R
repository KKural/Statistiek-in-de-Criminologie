...
if (graded_config_count != 51L) {
  stop(sprintf("Expected 51 active graded exercises after grouping, found %d.", graded_config_count))
}
...
if (length(evaluator_files) != 50L) {
  stop(sprintf("Expected 50 R evaluators plus one tested Python exercise, found %d R evaluators.",
...
for (evaluator_file in evaluator_files) {
  ...
  paths <- c(
    ...
    boilerplate = file.path(exercise_dir, "description", "boilerplate", "boilerplate"),
    config = file.path(exercise_dir, "config.json")
  )
  if (!all(file.exists(paths))) {
    failures <- c(failures, sprintf("%s: missing description, boilerplate, or config", relative_dir))
    next
  }

  description <- read_text(paths[["description"]])
  boilerplate <- read_text(paths[["boilerplate"]])
  ...
  if (length(fields) < 1L || length(fields) > 5L) {
    failures <- c(
      failures,
  ...
  multi_answer <- length(fields) >= 3L && length(fields) <= 5L &&
...
if (length(grouped_files) < 34L) {
  failures <- c(
    failures,
    sprintf("Expected at least 34 multi-answer evaluators with named expected_values, found %d",
...
cat(sprintf(
  paste0(
    ...
    "named answer fields and an explicit objective; every R activity has one testcase, ",
    "valid learner slots, a retained token, and the feedback contract.\n"
  ),
  graded_config_count,
  length(grouped_files)
))
