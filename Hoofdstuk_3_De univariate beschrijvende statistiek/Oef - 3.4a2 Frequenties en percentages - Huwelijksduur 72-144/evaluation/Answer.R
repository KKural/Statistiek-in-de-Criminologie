    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          results <- list(
            frequenties = check_vector("frequenties", c(1, 1, 1), 0),
            percentages = check_vector("percentages", c(9.09, 9.09, 9.09), 0.005)
          )
          assign("results_3_4a2", results, envir = globalenv())
          all(vapply(results, function(result) isTRUE(result$ok), logical(1)))
        },
        TRUE,
        comparator = function(generated, expected, ...) {
          results <- get("results_3_4a2", envir = globalenv())
