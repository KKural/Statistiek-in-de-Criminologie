    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          # Conclusion: r_XY goes from ~0.10 (trivial) to ~+0.72 (strong positive)
        ...
        comparator = function(generated, expected, ...) {
          ...
          for (key in names(qnames)) {
            ...
            if (!r$exists) {
              lines <- c(lines, paste0(
                "❌ **", lbl, "** — variabele `", key, "` niet ingevuld.\n\n",
                wrong_fns[[key]]("?"), "\n"
              ))
            } else if (r$correct) {
              lines <- c(lines, paste0(
                "✅ **", lbl, "**: correct (", as.character(r$value),
                ") — ", correct_msgs[[key]], "\n"
              ))
              score <- score + 1
            } else {
              ...
              lines <- c(lines, paste0(
                "❌ **", lbl, "** — jouw antwoord: **", as.character(r$value),
                "**\n\n", detail, "\n"
              ))
            }
          }
          ...
          contract_rule <- function(key) {
            k <- tolower(key)
            if (grepl("gemiddelde|_bar", k)) {
              return("Gemiddelde = som / n; gebruik n-1 alleen bij een steekproefvariantie.")
            }
            if (grepl("^ss|scp", k)) {
              return("Centreer eerst, vorm kwadraten of kruisproducten en sommeer; een SS/SCP wordt niet gedeeld.")
            }
            if (grepl("variantie|^var", k)) {
              return("Steekproefvariantie = SS / (n-1); houd SS, variantie en standaardafwijking uit elkaar.")
            }
            if (grepl("standaardfout", k)) {
              return("Standaardfout = standaardafwijking / wortel(n); gebruik de juiste onzekerheidsmaat.")
            }
            if (grepl("^sd|sigma", k)) {
              return("Een standaardafwijking is de positieve wortel van de variantie en behoudt de oorspronkelijke eenheid.")
            }
            if (grepl("cov|scp", k)) {
              return("Covariantie gebruikt de getekende gecentreerde kruisproducten en de afgesproken deler.")
            }
            if (grepl("teller", k)) {
              return("Werk de volledige teller met tekens en haakjes uit voordat je door de noemer deelt.")
            }
            if (grepl("noemer", k)) {
              return("Bereken beide factoren onder de wortel afzonderlijk; de noemer moet positief zijn.")
            }
            if (grepl("r_|pearson|partial", k)) {
              return("Een correlatie is schaalvrij en ligt tussen -1 en 1; koppel elke correlatie aan de juiste variabelen en controles.")
            }
            if (grepl("^bi_|interval|breder", k)) {
              return("Een betrouwbaarheidsinterval is schatting plus/minus kritieke waarde maal standaardfout; meer betrouwbaarheid geeft een breder interval.")
            }
            if (grepl("^df", k)) {
              return("Vrijheidsgraden volgen uit het aantal onafhankelijke informatiedelen of geschatte parameters; tel die eerst expliciet.")
            }
            if (grepl("^ms", k)) {
              return("Mean square = bijbehorende sum of squares / bijbehorende vrijheidsgraden.")
            }
            if (grepl("f_ratio|^f_", k)) {
              return("F = MS tussen / MS binnen; houd teller en noemer op hun juiste ANOVA-rij.")
            }
            if (grepl("eta|r_kwadraat", k)) {
              return("Een verklaarde proportie vergelijkt verklaarde met totale variatie en ligt tussen 0 en 1, of 0% en 100%.")
            }
            if (grepl("chi", k)) {
              return("Sommeer voor elke cel (O-E)^2/E met de verwachte, niet de geobserveerde, frequentie als deler.")
            }
            if (grepl("^e_", k)) {
              return("Verwachte frequentie = rijtotaal maal kolomtotaal / N.")
            }
            if (grepl("kritieke", k)) {
              return("Lees de kritieke waarde af met zowel het juiste alfa als de juiste vrijheidsgraden.")
            }
            if (grepl("significant|conclusie|richting|vraag|h0", k)) {
              return("Pas eerst de expliciete beslisregel toe en formuleer daarna alleen de conclusie die door die vergelijking wordt gedragen.")
            }
            if (grepl("n_minimum|n_voor|quotient", k)) {
              return("Bereken de steekproefgrootte met ongeafronde tussenwaarden en rond het eindresultaat altijd naar boven af.")
            }
          ...
          invalid_finite_choice <- function(key, value) {
            if (!(key %in% names(finite_choices))) {
              return(FALSE)
            }
