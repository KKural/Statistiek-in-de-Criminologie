    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          assign("results_3_4c", results, envir = globalenv())
          all(vapply(results, function(result) isTRUE(result$ok), logical(1)))
        },
        TRUE,
        comparator = function(generated, expected, ...) {
          results <- get("results_3_4c", envir = globalenv())
          deviations <- results$deviations
          squared <- results$squared
          if (isTRUE(generated == expected)) {
            message <- paste(
              "**Bevestiging:** de vier afwijkingen en vier kwadraten voor observaties 1 tot en met 4 zijn correct berekend met het exacte gemiddelde 2628/11 en ongeronde tussenwaarden.",
              "**Denkregel:** bereken met volledige precisie en rond alleen gerapporteerde eindwaarden.",
              "**Transferstap:** pas dezelfde precisieregel toe op het volgende observatieblok.",
              sep = "\n\n"
            )
          } else {
            if (!deviations$valid || !squared$valid) {
              ...
              next_step <- "Vul beide vectoren met vier eindige getallen voor observaties 1 tot en met 4."
