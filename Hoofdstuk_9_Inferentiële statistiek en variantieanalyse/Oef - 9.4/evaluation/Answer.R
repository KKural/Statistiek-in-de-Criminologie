    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          results$grand_mean        <- check_val("grand_mean",        exp_grand,         tol = 0.0050001)
          results$SS_within_groot   <- check_val("SS_within_groot",   exp_SS_w_groot,    tol = 0)
          results$SS_within_middel  <- check_val("SS_within_middel",  exp_SS_w_middel,   tol = 0)
          results$SS_within_ruraal  <- check_val("SS_within_ruraal",  exp_SS_w_ruraal,   tol = 0)
          results$SS_within         <- check_val("SS_within",         exp_SS_within,     tol = 0)
          results$df_within         <- check_val("df_within",         exp_df_within,     tol = 0)
          results$MS_within         <- check_val("MS_within",         exp_MS_within,     tol = 0.0050001)
          results$SS_between        <- check_val("SS_between",        exp_SS_between,    tol = 0.0050001)
          results$df_between        <- check_val("df_between",        exp_df_between,    tol = 0)
          results$MS_between        <- check_val("MS_between",        exp_MS_between,    tol = 0.0050001)
          results$F_ratio           <- check_val("F_ratio",           exp_F,             tol = 0.0050001)
          results$eta_kwadraat      <- check_val("eta_kwadraat",      exp_eta2,          tol = 0.0000501)
        ...
        comparator = function(generated, expected, ...) {
          ...
          wrong_msg_sig_anova <- function(val) {
            v <- parse_num(val)
            if (!is.na(v) && v == 2)
              return(sprintf("**Waarschijnlijke redenering:** je hebt mogelijk de F-vergelijking omgekeerd of de kritieke grens als minimum voor niet-significantie gelezen.\n\n**Waarom dit niet klopt:** F=%.2f is veel groter dan 3.35, dus het effect is significant en vraagt code 1.", ev$F))
          ...
          finite_choices <- list(significant_anova = 1:2)
