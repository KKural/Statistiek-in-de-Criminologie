    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          assign("results_3_3d5", results, envir = globalenv())
          all(vapply(results, function(result) isTRUE(result$ok), logical(1)))
        },
        TRUE,
        comparator = function(generated, expected, ...) {
          results <- get("results_3_3d5", envir = globalenv())
          deviations <- results$deviations
          squared <- results$squared
          if (isTRUE(generated == expected)) {
            message <- paste(
              "**Bevestiging:** de vier afwijkingen en vier gekwadrateerde afwijkingen voor observaties 17 tot en met 20 zijn correct.",
