    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          assign("results_3_4c2", results, envir = globalenv())
          all(vapply(results, function(result) isTRUE(result$ok), logical(1)))
        },
        TRUE,
        comparator = function(generated, expected, ...) {
          results <- get("results_3_4c2", envir = globalenv())
          deviations <- results$deviations
          squared <- results$squared
          if (isTRUE(generated == expected)) {
            message <- paste(
              "**Bevestiging:** de vier afwijkingen en vier kwadraten voor observaties 5 tot en met 8 zijn correct berekend met het exacte gemiddelde 2628/11 en ongeronde tussenwaarden.",
