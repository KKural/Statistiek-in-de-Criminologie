def evaluate_classification():
    ...
    for var_key in expected_answers.keys():
        ...
        if not result['exists']:
            ...
            feedback_parts.append(f"{counter}. **{var_display}**: {result['value']} ✅")
        else:
            feedback_parts.append(f"{counter}. **{var_display}**: {result['value']} ❌")
        counter += 1
    ...
    if all_correct:
        ...
        if incorrect_vars:
            ...
            for var_key in incorrect_vars:
                ...
                if not result['exists']:
                    feedback_parts.append(f"• **{var_display}**: ❌ Variabele niet gevonden. Vergeet je de variabele te definiëren? Gebruik: `{var_key} = \"{result['expected']}\"`")
                else:
    ...
    feedback_parts.append("• **Nominaal**: Categorieën zonder ordening (bijv. kleuren, geslacht)")
    feedback_parts.append("• **Ordinaal**: Categorieën met ordening (bijv. schoolcijfers: slecht < goed < uitstekend)")
    feedback_parts.append("• **Interval**: Getallen met gelijke afstanden, geen echt nulpunt (bijv. temperatuur in °C)")
    feedback_parts.append("• **Ratio**: Getallen met gelijke afstanden én echt nulpunt (bijv. leeftijd, gewicht)")
