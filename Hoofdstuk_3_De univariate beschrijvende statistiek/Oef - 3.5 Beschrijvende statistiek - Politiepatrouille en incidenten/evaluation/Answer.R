    ...
    "",
    {
      testEqual(
        ...
        function(env) {
          ...
          # Normalise student input before grading. Invalid input must result in
          # ordinary incorrect-answer feedback, never an evaluator error.
          as_numeric_safely <- function(value) {
            suppressWarnings(
              tryCatch({
                numeric_value <- as.numeric(value)
                if (length(numeric_value) == 0L) NA_real_ else numeric_value
          ...
          is_finite_vector <- function(value, expected_length) {
            is.numeric(value) &&
              length(value) == expected_length &&
              all(is.finite(value))
          }

          as_choice_safely <- function(value) {
            tryCatch({
              choice <- as.character(value)
              if (length(choice) == 1L && !is.na(choice) && nzchar(trimws(choice))) {
                choice
              } else {
                NA_character_
              }
          ...
          expected_sd <- 2.74  # round(sqrt(7.5), 2)
          
          # STEP 1.1: GEMIDDELDE
          if (exists("gemiddelde_incidenten", envir = env)) {
            current_val <- as_numeric_safely(get("gemiddelde_incidenten", envir = env))
            results$gemiddelde_incidenten <- list(
              exists = TRUE,
              value = current_val,
              correct = is_finite_scalar(current_val) && abs(current_val - expected_mean) < 0.01,
          ...
          if (exists("afwijkingen", envir = env)) {
            current_val <- as_numeric_safely(get("afwijkingen", envir = env))
            
            results$afwijkingen <- list(
              exists = TRUE,
              value = current_val,
              correct = is_finite_vector(current_val, length(expected_deviations)) && all(abs(current_val - expected_deviations) < 0.01),
          ...
          if (exists("gekwadrateerde_afwijkingen", envir = env)) {
            current_val <- as_numeric_safely(get("gekwadrateerde_afwijkingen", envir = env))
            
            results$gekwadrateerde_afwijkingen <- list(
              exists = TRUE,
              value = current_val,
              correct = is_finite_vector(current_val, length(expected_squared)) && all(abs(current_val - expected_squared) < 0.01),
          ...
          if (exists("sum_of_squares", envir = env)) {
            current_val <- as_numeric_safely(get("sum_of_squares", envir = env))
            
            results$sum_of_squares <- list(
              exists = TRUE,
              value = current_val,
              correct = is_finite_scalar(current_val) && abs(current_val - expected_ss) < 0.01,
          ...
          if (exists("variantie_incidenten", envir = env)) {
            current_val <- as_numeric_safely(get("variantie_incidenten", envir = env))
            
            results$variantie_incidenten <- list(
              exists = TRUE,
              value = current_val,
              correct = is_finite_scalar(current_val) && abs(current_val - expected_variance) < 0.01,
          ...
          if (exists("standaarddeviatie_incidenten", envir = env)) {
            current_val <- as_numeric_safely(get("standaarddeviatie_incidenten", envir = env))
            
            results$standaarddeviatie_incidenten <- list(
              exists = TRUE,
              value = current_val,
              correct = is_finite_scalar(current_val) && abs(current_val - expected_sd) <= 0.005,
          ...
          if (exists("gekozen_spreidingsmaat", envir = env)) {
            current_val <- as_choice_safely(get("gekozen_spreidingsmaat", envir = env))
            
            results$gekozen_spreidingsmaat <- list(
              exists = TRUE,
              value = current_val,
              correct = !is.na(current_val) && tolower(current_val) %in% c("standaarddeviatie", "standard_deviation", "sd", "standaardafwijking"),
            ...
            if(any(incorrect_vars)) {
              ...
              if(!results$gemiddelde_incidenten$correct) {
                if(!results$gemiddelde_incidenten$exists) {
                  ...
                  if(length(student_answer) != 1L || !is.finite(student_answer)) {
                    feedback_parts <- c(feedback_parts, "• **1.1 Gemiddelde**: vul één eindig numeriek getal in, zonder tekst, `NA`, `Inf` of extra waarden.")
                  } else if(abs(student_answer - 5.4) < 0.01) {
              ...
              if(!results$afwijkingen$correct) {
                if(!results$afwijkingen$exists) {
                  ...
                  if(length(student_answer) == 1) {
                    feedback_parts <- c(feedback_parts, "• **2.1 Afwijkingen**: vul een numerieke vector met precies negen eindige waarden in, één afwijking per wijk.")
                  } else if(length(student_answer) == 9L && all(is.finite(student_answer)) && all(student_answer >= 0) && sum(abs(student_answer - abs(c(-4, 1, 4, -1, 0, 2, -3, -2, 3)))) < 1) {
                  ...
                  } else if(length(student_answer) != 9L || !all(is.finite(student_answer))) {
                    feedback_parts <- c(feedback_parts, "• **2.1 Afwijkingen**: vul een numerieke vector met precies negen eindige waarden in, één afwijking per wijk.")
                  } else {
              ...
              if(!results$gekwadrateerde_afwijkingen$correct) {
                if(!results$gekwadrateerde_afwijkingen$exists) {
                  ...
                  if(length(student_answer) == 1) {
                    feedback_parts <- c(feedback_parts, "• **2.2 Gekwadrateerde afwijkingen**: vul een numerieke vector met precies negen eindige waarden in, één kwadraat per wijk.")
                  } else if(length(student_answer) == 9L && all(is.finite(student_answer)) && all(abs(student_answer - c(2,7,10,5,6,8,3,4,9)^2) < 0.01)) {
              ...
              if(!results$sum_of_squares$correct) {
                if(!results$sum_of_squares$exists) {
                  ...
                  if(length(student_answer) != 1L || !is.finite(student_answer)) {
                    feedback_parts <- c(feedback_parts, "• **2.3 Som gekwadrateerde afwijkingen**: vul één eindig numeriek getal in, zonder tekst, `NA`, `Inf` of extra waarden.")
                  } else {
              ...
              if(!results$variantie_incidenten$correct) {
                if(!results$variantie_incidenten$exists) {
                  ...
                  if(length(student_answer) != 1L || !is.finite(student_answer)) {
                    feedback_parts <- c(feedback_parts, "• **3.1 Variantie**: vul één eindig numeriek getal in, zonder tekst, `NA`, `Inf` of extra waarden.")
                  } else if(abs(student_answer - 6.67) < 0.01) {
              ...
              if(!results$standaarddeviatie_incidenten$correct) {
                if(!results$standaarddeviatie_incidenten$exists) {
                  ...
                  if(length(student_answer) != 1L || !is.finite(student_answer)) {
                    feedback_parts <- c(feedback_parts, "• **3.2 Standaarddeviatie**: vul één eindig numeriek getal in, zonder tekst, `NA`, `Inf` of extra waarden.")
                  } else if(abs(student_answer - 7.5) < 0.01) {
              ...
              if(!results$gekozen_spreidingsmaat$correct) {
                if(!results$gekozen_spreidingsmaat$exists) {
                  ...
                  if(length(student_answer) != 1L || is.na(student_answer) || !nzchar(trimws(student_answer))) {
                    feedback_parts <- c(feedback_parts, "• **4.1 Gekozen spreidingsmaat**: vul één tekstkeuze tussen aanhalingstekens in.")
                  } else if(student_answer %in% c("variantie", "variance")) {
